...
# 生图服务专用 Key：优先读取独立配置，其次兼容 ARK_API_KEY，最后回退到通用 ARK Key
VOLCENGINE_IMAGE_ARK_API_KEY = os.getenv(
    'VOLCENGINE_IMAGE_ARK_API_KEY',
    os.getenv('ARK_API_KEY', VOLCENGINE_ARK_API_KEY),
).strip()
...
VOLCENGINE_IMAGE_MODEL = os.getenv('VOLCENGINE_IMAGE_MODEL', 'doubao-seedream-5-0-260128')  # 图片生成模型：doubao-seedream-5-0-260128（默认）
